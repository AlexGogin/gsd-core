...
| `statusline.show_git` | boolean | `false` | Append a git segment after the directory: current branch plus compact work-state markers (`+staged` `~unstaged` `?untracked` `↑ahead` `↓behind`, or `✓` when clean and in sync). One `git status --porcelain=v2` call per render; the segment is absent outside a git repo or when git is unavailable. Also enables the STATE.md freshness marker (`state ~N commits back`) when STATE.md carries a `state_head` stamp — one extra `git rev-list --count` per render, so a user who has not opted into git pays nothing |
